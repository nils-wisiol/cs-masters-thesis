...
and Christian Glaﬂer (Universit‰t W¸rzburg, Germany), committee member
...
in between these two theories. The core of the thesis is the implication
chart in Figure \ref{fig:implication-chart} that summarizes virtually
all results mentioned in this thesis.

Section \ref{sec:Preliminaries} is split into two pieces. In \ref{sub:Disjoint--Pairs},
...
between both theories that was discovered by Razborov. It presents
a proof for Razborov's theorem that uses notions of complexity theory.

We also take a look at relativized worlds in Section \ref{sec:Relativized-Worlds}.
We point the reader to oracles relative to which optimal proof systems
exist, and respectively, do not exist. Section \ref{sub:Converse-of-Razborov}
...
Finally we refer the reader to an oracle that separates different
refinements of the ESY-conjecture from each other.
...
\path[every node/.style={sloped,anchor=south,auto=false}]
  ...
  (PKCS) edge node{\footnotesize\cite{journals/siamcomp/GrollmannS88}} (ESYTfails)
;
\end{tikzpicture}
\end{center}
...
The study of disjoint $\NP$-pairs originates in the study of public-key
cryptosystems (PKCS). The interest in secure PKCS is fundamental to
everyday life as well as to academia, as provably hard-to-crack PKCS
would imply $\NP\neq\P$.

To study the hardness of PKCS, Even, Selman and Yacobi \cite{journals/iandc/EvenSY84}
used the notion of \emph{promise problems} rather than decision problems
to model the problem of cracking a PKCS. In fact, promise problems
are a generalization of decision problems. A machine working on a
promise problem is not only given an input, but also a promise that
for this input, a certain condition holds. The machine \emph{solves}
the problem, if it gives the right answer on all inputs for which
the promise holds. If the promised condition does in fact not hold
for a given input, then the machine can act arbitrarily.
...
\end{enumerate}
A machine that \emph{solves} such a promise problem has to accept
on all Yes-Instances, to reject on all No-Instances and act arbitrarily
on all other strings. This includes that the machine might not halt
at all.
...
the hardness of cracking PKCS, that is, captures the hardness of finding
the plain text to a given cipher text $C$ and public key $K$. To
crack the cryptosystem, we will conduct a binary search among all
strings up to a reasonable length. The scope for the binary search
is limited, as the length of the plain text is polynomial in the length
of the cipher text. Notice that this notion captures the hardness
to crack \emph{every} cipher text in a cryptosystem. While we can
conclude cryptographic insecurity from an easy cracking problem, a
hard cracking problem does not imply cryptographic security, as a
subset of cipher texts may be still easy to crack. 
...
$M'$. Thus, the runtime of cracking the public-key cryptosystem is
within a logarithmic factor of the runtime of the machine solving
the promise problem. Therefore, we consider the hardness of the promise
problem as a good measurement for the hardness of the cracking problem.
...
correctly, assume we have a cryptosystem such that the \emph{decision
}problem $A$ is not efficiently computable. However, if there is
an algorithm efficiently solving the promise problem $(A,B)$, the
crypto system would still be easy to crack. On the other hand, if
the promise problem is hard, the decision problem will also be.

We call a set $S$ for which $A\subseteq S$ and $B\subseteq\overline{S}$
\emph{a separator.} Let the set $\Sep(A,B)$ denote the set of all
separators for a given pair $(A,B)$. A pair $(A,B)$ that has no
polynomial-time decidable set in $\Sep(A,B)$ is called \emph{$\P$-inseparable},
otherwise it is called \emph{$\P$-separable}.
...
\end{enumerate}
\end{defn}
A promise problem $(A,B)$, with $A,B$ non-empty and $A,B\in\NP$
is a \emph{disjoint $\NP$-pair}. We define $\DisjNP$ to be the set
of all disjoint $\NP$-pairs. For this class, we define completeness:
\begin{defn}
...
\end{conjecture}
If the conjecture holds, then no public-key cryptosystem is $\NP$-hard
to crack. The following refined version of the ESY-conjecture can
be proven to be equivalent to $\NP\neq\coNP$, see Theorem \ref{thm:equiv-esymfails-np=00003Dconp-polyboundpps}.
\begin{conjecture}[ESY-m]
 For every pair of disjoint sets in $\NP$, there is a separator
that is not many-one-hard for $\NP$. \cite{conf/icalp/HughesPRS12} 
\end{conjecture}
We will study consequences of the ESY-conjectures in Section \ref{sub:ESY-conjectures}.


\subsection{Propositional Proof Systems\label{sub:Propositional-Proof-Systems}}

To start with an example, we will have a look at the \emph{resolution
principle}, which was introduced by Robinson \cite{journals/jacm/Robinson65}.
...
\end{cases}
\]
The function $f$ is polynomial-time computable. By the Resolution
...
is a tautology. We call $f$ a propositional proof system, and we
call $\langle\varphi,w\rangle$ an $f$-proof for $\neg\varphi$.
\begin{defn}
...
\footnote{At this point it is worth to mention that most literature uses a generalization
of this term of proof systems. A function $f$ is a \emph{proof system
for $L$}, if $f$ is onto $L$ and polynomial-time computable. Notion
from the definition above can be used accordingly. This thesis only
uses the \emph{propositional proof systems for $\TAUT$}, or short,
\emph{proof systems}.%
}
\end{defn}
...
no known proof system has been proven to have proofs with length bounded
...
if there is a function $h$ such that for all $w$, it holds that
$f(h(w))=g(w)$ and $|h(w)|\leq p(|w|)$. We call $h$ a \emph{translation
function}. If $h$ is polynomial-time computable, we say $f$ \emph{p-simulates}
$g$. A proof system that simulates (p-simulates) every other proof
system is called \emph{optimal} (\emph{p-optimal}).
\end{defn}
...
systems exist, it is also unknown whether optimal or even p-optimal
proof systems exist. To study the existence of optimal and p-optimal
proof systems, we will therefore study sufficient conditions and implications
in Section \ref{sec:Propositional-Proof-Systems}. To become familiar
with these notions, we present a strong sufficient condition for the
...
\begin{proof}
Let $N$ be a $\NP$-machine deciding the set of tautologies, $\TAUT\in\coNP$.
We define $f$ by 
\[
...
\end{cases}
\]
Notice that a proof system does not have to be a total function. By
definition, $f$ outputs only tautologies, and for every tautology
there is an accepting path of $N$, so $f$ is onto $\TAUT$.
...
$i$ of $N$. Notice that $g$ might not by polynomial-time computable,
but is polynomially length-bounded. Let $w$ be an $f'$-proof for
$x$. With $g$, we can translate $w$ into $\langle g(w),f'(w)\rangle$,
which is an $f$-proof for $x$.
\end{proof}
...
the following Lemmas. A comprehensive proof in in the paper of Glaﬂer,
...
are equivalent. This result was first proven by Grollmann and Selman
\cite{journals/siamcomp/GrollmannS88}.
\begin{lem}
Let $(A,B)$ and $(C,D)$ be disjoint pairs. Then the following assertions
are equivalent:
\begin{enumerate}
\item $(A,B)\predt(C,D)$
...
\end{enumerate}
\end{lem}
Therefore, for the rest of this thesis, we will only use the notions
of many-one and Turing reducibility.
...
\end{enumerate}
\end{lem}
This list can be extended to other reductions as well. In this thesis,
we mention these specific reductions because there are known results
that relate to these assertions.
...
since $C\in\NP$ and $S$ is $r$-hard for $\NP$, we have $C\redr S$.
This proves $C$, which is a separator of $(C,D)$, reduces to any
separator of $(A,B)$. By definition \ref{def:pair-reduction}, we
have $(C,D)\predr(A,B)$ and thus $(A,B)$ is $r$-complete for $\DisjNP$.
\end{proof}
As mentioned above, the refinements of the (original) ESY-$T$ have
interesting relations to computational complexity as well. The ESY-$m$
conjecture is equivalent to $\NP\neq\coNP$, and ESY-$tt$ implies
$\NP\neq\UP$.
\begin{thm}
...
proof system. We call a proof system $f$ \emph{optimal}, if $f$-proofs
...
conditions for the existence of optimal proof systems. There are oracles
relative to which optimal proof systems exist and relative to which
optimal proof systems do not exist, see Section \ref{sub:Existence-Optimal-and-p-optimal-ps}.
...
proof systems exist, first KrajÌ\v{c}ek and Pudl·k \cite{journals/jsyml/KrajicekP89}
and later Meﬂner and Tor·n proved sufficient conditions for the existence
...
We define $\EE$ to be the class of languages that can be decided
in time $2^{O(2^{n})}$. A language $L$ is called \emph{almost-tally},
if every string in $L$ is of the form $0^{*}10^{*}$. By $\powerset(0^{*}10^{*})$
we denote the class of all almost-tally languages. Meﬂner and Tor·n
use the notion of almost-tally languages to obtain an even weaker
sufficient condition than mentioned in the chart:
\begin{thm}
...
refer to the original paper by Meﬂner and Tor·n \cite{journals/eccc/ECCC-TR97-026}.
...
in $|w|$. The Gˆdel-number $i$ is a constant in $|w|$. Parameter
...
of $\NEEE=\coNEEE$. However, in the case of triple exponential time,
the output $w'$ of the translation function is not within polynomial
bounds of $w$, as the length of the computation path $\alpha$ is
\[
...
In this section, we will present a result of Kˆbler, Meﬂner and Tor·n
...
oracle. As a further note, KrajÌ\v{c}ek and Pudl·k \cite{journals/jsyml/KrajicekP89,journals/iandc/KoblerMT03}
...
therefore the result by Kˆbler, Meﬂner and Tor·n provides evidence
...
\end{enumerate}
\end{thm}
For the proof, we refer the reader to the work of Kˆbler, Meﬂner and
...
We refer the reader to Glaﬂer, Selman and Zhang \cite{conf/birthday/GlasserSZ06}
who provide a proof that disjoint $\NP$-pairs and canonical pairs
for proof systems have the same degree structure.
...
Fortnow and respectively Meﬂner and Tor·n found oracles relative to
which there is no optimal respectively no p-optimal proof system.
Previously, Meﬂner and Tor·n proved \cite{journals/eccc/ECCC-TR97-026}
...
Glaﬂer, Selman, Sengupta and Zhang \cite[Chapter 6]{journals/eccc/ECCC-TR03-011}
...
The oracles $O_{1}$ and $O_{2}$ by Glaﬂer, Selman, Sengupta and
...
but Glaﬂer et al. proved that it cannot be answered with a relativizable
...
Glaﬂer and Wechsung constructed an oracle $D$ relative to which $\UP=\NP$
...
\section{Open Questions and Future Work\label{sec:Conclusion}}

All major questions raised in this thesis remain unsolved. In particular,
for all assertions shown in Figure \ref{fig:implication-chart} it
is not known if they are true or false. For any of the implications
shown in the chart, it is interesting to find oracles such that the
converse of an implication holds or does not hold. For the previously
mentioned oracle $D$ by Glaﬂer and Wechsung \cite{DBLP:journals/jucs/GlasserW03},
it is an interesting question to see if there exist many-one complete
pairs relative to $D$, since oracle $D$ implies that $\NP\neq\text{\ensuremath{\coNP}}$.
The most significant converse to study is the question whether the
existence of many-one complete disjoint $\NP$-pairs implies the existence
of optimal proof systems. As stated in Section \ref{sub:Converse-of-Razborov},
this question can not be answered with a relativizable proof.
