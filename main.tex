...
\thanks{\copyright C. Glaßer, A. Hughes, A. L. Selman, N. Wisiol, 2014%
} }


\author{C. Glaßer%
\thanks{University at Würzburg, Germany, \texttt{glasser@informatik.uni-wuerzburg.de}%
}, A. Hughes%
\thanks{University at Buffalo, New York, \texttt{ahughes6@buffalo.edu}%
}, A. L. Selman%
\thanks{University at Buffalo, New York, \texttt{selman@buffalo.edu}%
}, N. Wisiol%
\thanks{University at Buffalo, New York, \texttt{mail@nils-wisiol.de}%
}\\
\\
\includegraphics[height=45mm,bb = 0 0 200 100, draft, type=eps]{pics/Christian.jpg}~\includegraphics[height=45mm,bb = 0 0 200 100, draft, type=eps]{pics/Andrew.jpg}~\includegraphics[height=45mm,bb = 0 0 200 100, draft, type=eps]{pics/Alan.jpg}~\includegraphics[height=45mm,bb = 0 0 200 100, draft, type=eps]{pics/Nils.jpg}}
...
\subsection{Polynomially-bounded propositional proof systems and $\NP=\coNP$}
...
\begin{proof}
Let $\NP=\coNP$. Then there is a $\NP$-machine $M$ such that there
is an accepting path $w$ in the computation of $M$ for every tautology
$\varphi$. This allows us to define a proof system $f$, in which
all proofs are polynomially length bounded:
\[
f(\langle\varphi,w\rangle)=\begin{cases}
...
\end{cases}
\]
As $f$ only considers one path in the computation of $M$, it is
polynomial-time computable, and the fact that $f$ only outputs tautologies
together with the fact that there is a proof for every tautology makes
$f$ a proof system. Moreover, for every tautology $\varphi$ there
is an accepting path $w$ in $M$ which is polynomially-bounded. Therefore,
all proofs are polynomially length bounded.

To proof the converse, let $f$ by a polynomially-bounded proof system.
We will show that $\TAUT\in\NP$, from which follows that $\NP=\coNP$.
Let $M$ be a nondeterministic Turing machine such that on input $\varphi$,
$M$ guesses a $f$-proof $w$ and calculates $f(\langle\varphi,w\rangle)$.
The machine then accepts if and only if $f(\langle\varphi,w\rangle)=\varphi$.
Hence, every accepted string is a tautology, and every tautology is
accepted.
\end{proof}
...
factor). If we could prove the existence of a optimal proof system
...
of any length exist, which is why we are interested in necessary and
...
$|w|$. The Gödel-number $i$ is a constant in $|w|$. The computation
...
to Köbler et al \cite{journals/iandc/KoblerMT03}. However our interest
